  ...
  try {
    ...
    requestAnimationFrame(async () => {
      if (url.includes('create.html')) {
        ...
        const { mountRegisterView } = await import('../views/register.js');
        await waitForElement('#register-app');
        mountRegisterView();
      } else if (url.includes('login.html')) {
        const { mountLoginView } = await import('../views/login.js');
        await waitForElement('#login-app');
        mountLoginView();
      }
    });
  } catch (err) {
